  ...
  if (!stats && !error) {
    return (
      <div className="flex flex-col items-center justify-center h-full p-8 text-center space-y-6 animate-in fade-in duration-700">
        <div className="w-24 h-24 rounded-3xl bg-cyan-600/10 flex items-center justify-center text-cyan-400 shadow-xl shadow-cyan-500/10 border border-cyan-500/20">
        ...
        <div className="flex items-center gap-2 text-cyan-400 font-bold animate-bounce mt-4">
  ...
  const pluginCategories = [
    {
      ...
      name: 'All Plugins',
      plugins: [
        { id: 'dashboard', name: 'Dashboard', Icon: Home, description: 'View server statistics and overview', enabled: true, path: '/dashboard' },
        { id: 'welcome', name: 'Welcome & Goodbye', Icon: Inbox, description: 'Send custom messages when members join or leave', enabled: true, path: '/dashboard/welcome-setup/' + selectedGuild?.id },
        { id: 'tickets', name: 'Tickets', Icon: Ticket, description: 'Create and manage support tickets', enabled: true, path: '/dashboard/tickets' },
        { id: 'leveling', name: 'Levels', Icon: BarChart3, description: 'Track member activity and assign ranks', enabled: true, path: '/dashboard/leveling' },
        { id: 'giveaways', name: 'Giveaways', Icon: Gift, description: 'Host exciting giveaways for your community', enabled: true, path: '/dashboard/giveaways' },
  ...
  return (
    <div className="min-h-screen bg-[#0f172a] relative">
      ...
      <div className="pointer-events-none absolute inset-0 overflow-hidden">
        <div className="absolute -top-40 -left-40 w-[600px] h-[600px] rounded-full bg-cyan-600/5 blur-[160px]"></div>
        <div className="absolute -bottom-40 -right-40 w-[600px] h-[600px] rounded-full bg-blue-600/5 blur-[160px]"></div>
      </div>

      <div className="relative z-10">
        {/* Top Banner with Promo */}
        <div className="bg-gradient-to-r from-cyan-600/20 via-blue-600/20 to-purple-600/20 border-b border-white/10 px-6 py-8 md:py-10">
          <div className="max-w-7xl mx-auto">
            <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-6">
              ...
              <div className="flex items-center gap-3">
                <div className="px-4 py-2 rounded-xl bg-cyan-600/20 text-cyan-400 text-sm font-bold border border-cyan-500/30">
        ...
        <div className="max-w-7xl mx-auto px-6 py-8 md:py-12">
          ...
          <div className="mt-12 pt-8 border-t border-white/10">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              <InfoCard label="Server Name" value={stats?.serverName || 'N/A'} Icon={Home} />
              <InfoCard label="Total Channels" value={stats?.channels || 0} Icon={MessageSquare} />
              <InfoCard label="Total Roles" value={stats?.roles || 0} Icon={Shield} />
            </div>
          </div>
        </div>
      </div>
  ...
  return (
    <div className={`glass rounded-2xl p-6 border-l-4 ${borderColor} hover:translate-y-[-4px] transition-all duration-300 group`}>
      <div className="flex items-center justify-between mb-4">
        <div className={`w-12 h-12 rounded-xl bg-gradient-to-br ${color} flex items-center justify-center ${textColor}`}>
...
function PluginCard({ plugin, onNavigate }) {
  return (
    <button
      onClick={onNavigate}
      className="group relative rounded-2xl bg-gradient-to-br from-white/8 to-white/3 border border-white/10 p-6 hover:border-cyan-500/40 hover:bg-cyan-600/10 transition-all duration-300 overflow-hidden text-left"
    >
      {/* Background glow on hover */}
      <div className="absolute inset-0 bg-gradient-to-br from-cyan-600/0 to-blue-600/0 group-hover:from-cyan-600/10 group-hover:to-blue-600/10 transition-all duration-300 pointer-events-none"></div>

      <div className="relative z-10">
        {/* Icon */}
        <div className="w-12 h-12 rounded-xl bg-cyan-600/20 flex items-center justify-center mb-4 text-cyan-400 group-hover:scale-110 group-hover:bg-cyan-600/30 transition-all duration-300">
        ...
        <div className="flex items-center justify-between">
          ...
          <ArrowRight className="w-5 h-5 text-slate-400 group-hover:text-cyan-400 transition-colors group-hover:translate-x-1 duration-300" />
        </div>
      </div>
    </button>
  );
