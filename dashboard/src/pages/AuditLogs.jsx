...
export default function AuditLogs({ selectedGuild }) {
  ...
  const formatTimestamp = (ts) => {
    try {
      ...
      if (diffMin < 1) return t('auditLogs.justNow');
      if (diffMin < 60) return t('auditLogs.minutesAgo', { minutes: diffMin });
  ...
  return (
    <div className="p-4 md:p-8 max-w-7xl mx-auto space-y-6 md:space-y-8 animate-fade-in">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h1 className="text-2xl md:text-4xl font-black text-white tracking-tight">{t('auditLogs.headerTitle')}</h1>
          <p className="text-slate-400 mt-1 text-sm md:text-base">{t('auditLogs.headerSubtitle')}</p>
        </div>
        <div className="flex items-center gap-3">
          ...
          <button onClick={fetchLogs} className="p-2 hover:bg-white/5 rounded-xl transition-colors" title={t('auditLogs.refresh')}>
      ...
      {filteredLogs.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-20 text-center glass rounded-2xl">
          ...
          <p className="text-lg font-bold text-slate-400">{t('auditLogs.noLogsFound')}</p>
          <p className="text-sm text-slate-500 mt-1">{filter !== 'all' ? t('auditLogs.tryDifferentFilter') : t('auditLogs.noRecentActions')}</p>
        </div>
      ) : (
                ...
                <div className="flex items-center gap-3 sm:gap-4 flex-1 min-w-0">
                  ...
                  <div className="min-w-0 flex-1">
                    <p className="text-sm text-white">
                      ...
                      <span className="text-slate-400 mx-1.5">{t('auditLogs.performed')}</span>
                      <span className={`inline-block px-2 py-0.5 rounded-lg text-xs font-bold border ${colorClass}`}>{log.action}</span>
                    </p>
                    <p className="text-xs text-slate-500 mt-0.5 truncate">
                      {t('auditLogs.target')} <span className="text-slate-400">{log.target}</span>
                      {log.reason && <span className="ml-2 text-slate-500">- {t('auditLogs.reason')} {log.reason}</span>}
                    </p>
                  </div>
                </div>
      ...
      <p className="text-xs text-slate-600 text-center">
        {t('auditLogs.showingEntries', { filteredCount: filteredLogs.length, totalCount: logs.length })}
      </p>
    </div>
  );
}
