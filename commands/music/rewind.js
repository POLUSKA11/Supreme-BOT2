...
const { buildErrorEmbed, formatDuration, createProgressBar, COLORS } = require('../../utils/musicPlayer');
...
module.exports = {
    data: new SlashCommandBuilder()
        ...
        .addStringOption(opt =>
            opt.setName('time')
                .setDescription('Time to rewind (e.g. 30s, 1m, 1m30s). Default: 30 seconds')
                .setRequired(false)
        ),
    ...
    async execute(interaction) {
        ...
        const newPosition = Math.max(0, current - ms);
        ...
        const embed = new EmbedBuilder()
            ...
            .setAuthor({ name: '⏪ Rewound', iconURL: interaction.client.user.displayAvatarURL() })
            .setTitle(track.title.substring(0, 256))
            .setDescription(`\`${progress}\``)
            .addFields(
                { name: '⏪ Rewound By', value: `\`${formatDuration(ms)}\``, inline: true },
                { name: '📍 New Position', value: `\`${formatDuration(newPosition)} / ${formatDuration(track.durationMS)}\``, inline: true },
            )
            .setFooter({ text: `Rewound by ${interaction.user.tag}`, iconURL: interaction.user.displayAvatarURL() })
            .setTimestamp();
